 ...
 // State for gallery items
   let sliderItems = [];
    ...
    async function fetchSlide() {
        try {
          isLoadingslide = true;
            ...
            const response = await fetch('https://admin.radiotreichville.ci/radio_treich/sliders.php');
            ...
            slides = data.map((item, index) => ({
                url: "https://admin.radiotreichville.ci/pub/"+item.images, 
                lien: item.lien,
                title: item.titre,
    ...
    onMount(async()=>{
        ...
        try {
            const response = await fetch('https://admin.radiotreichville.ci/radio_treich/news3.php');
    ...
    onMount(async()=>{
        ...
        try {
            const response = await fetch('https://admin.radiotreichville.ci/radio_treich/events3.php');
    ...
    async function fetchGalleryItems() {
        try {
            ...
            // Utiliser l'API JSONPlaceholder pour simuler un fetch de données
            const response = await fetch('https://admin.radiotreichville.ci/radio_treich/animateurs');
            ...
            galleryItems = data.map((item, index) => ({
                src: "https://admin.radiotreichville.ci/animateurs/"+item.photo, 
                alt: item.nom,
                title: item.nom,
...
<svelte:head>
  <title>Radio Treichville - Bienvenue</title>
...
<div style="text-align: center;">
  ...
  <section class="section">
    <div style="paddding:5px 20px;box-sizing:border-box">
      <small><i class="bi bi-chevron-right"></i> Publicité </small>
...
<div style="background: #03800a;;width: 100%;" class="">
