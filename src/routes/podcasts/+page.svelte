  ...
  async function loadData() {
    ...
    try {
      ...
      const [paginatedResponse, allNewsResponse] = await Promise.all([
        fetch(`https://admin.radiotreichville.ci/radio_treich/podcasts.php?page=${currentPage}&limit=${itemsPerPage}`),
        fetch(`https://admin.radiotreichville.ci/radio_treich/allpodcasts.php`)
      ]);
